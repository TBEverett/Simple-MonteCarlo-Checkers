...
void build_board(short* board, short N, short *n_fichas_player, short*n_fichas_rival){ 
    short filas_con_fichas = (N-2)/2;
...
void printBoard(short* board, short N){
...
__host__ __device__ float win(short *board, short N){
    for(short i = 0; i < N; i++){
       for(short j = 0; j < N; j++){
            if (board[i*N + j] == 2){
                if(i == (N - 1)) 
...
Move player_select_move(Movimientos* movimientos, short N){
    char letras[] = {'A','B','C','D','E','F','G','H','K','L','M','N'};
...
__host__ __device__ void execute_movement(short* &board, short N, Move movimiento, short* n_fichas){
...
__host__ __device__ Movimientos* generarMovimientos(short* board, short N, short n_fichas, short ficha_aliada, Movimientos* movimientos){ //n_fichas (de 1 jugador) nos servirá para dejar de buscar cuando hayamos procesado todas las fichas
    short colum;

    short aux_numero_fichas = n_fichas; //Acotamos la cantidad de movimientos (cantidad de fichas * 2)
...
float MonteCarloSimulation(short* board,short N,Move movimiento, short n_fichas_player, short n_fichas_IA){
    ...
    while(true){
        ...
        short random = rand() % movimientos->length;
