...
__global__ void kernel(short* board, short N, short start_position, short end_position, short kill, short n_fichas_player, short n_fichas_IA, float* evaluacion_total){
    ...
    for(short i = 0; i < N*N; i++) local_board[i] = board[i];
    ...
    short turno_jugador = 1; //turno_jugador 1 es del jugador
...
int main(short argc, char** argv) { 
    ...
    short N = atoi(argv[1]);
    short gs = atoi(argv[2]) / 256 + 1;
    short bs = atoi(argv[2]) / gs;
    short verbose = atoi(argv[3]);
    short CPUorGPU = atoi(argv[4]); //0 o 1 para CPU o GPU respectivamente

    printf("%d %d", gs,bs);

    // Variables que trabajaremos 
    short* board = new short[N*N];
    short n_fichas_player = 0;
    short n_fichas_IA = 0;
    ...
    while(!flag_finalizado){
        ...
        if (movimientos->length == 0){
            printBoard(board, N);
            printf("Ha ganado la shorteligencia Articial. La era del hombre ha llegado a su fin");
        ...
        short indice_maximo = 0;
        float eval_maxima = 0.;
        float* evaluacion = new float; 
        float* evaluacionGPU;
        cudaMalloc((void**)&evaluacionGPU, sizeof(float));
        short* boardGPU;
        cudaMalloc((void**)&boardGPU, N * N * sizeof(short));
        cudaMemcpy(boardGPU, board, N * N * sizeof(short), cudaMemcpyHostToDevice);
        for(short i = 0; i < movimientos->length; i++){
            *evaluacion = 0; 
            short start_position =  movimientos->listaMovimientos[i].start_position;
            short end_position =  movimientos->listaMovimientos[i].end_position;
            short kill = movimientos->listaMovimientos[i].kill;
            ...
            if(CPUorGPU == 1){
                ...
                t1 = clock();
                for(short j = 0; j < bs*gs; j++) *evaluacion += MonteCarloSimulation(board, N, movimientos->listaMovimientos[i], n_fichas_player, n_fichas_IA);
                t2 = clock();
                float ms = 1000.0 * (float)(t2 - t1) / CLOCKS_PER_SEC;
                time += ms;
            }
